...
class CartItems extends HTMLElement {
  ...
  updateQuantity(line, quantity, productId, name, variantId) {
  ...
  updateLineItemProperties(line, properties,productId) {
    const body = JSON.stringify({
      line,
      properties,
      sections: this.getSectionsToRender().map((section) => section.section),
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        const parsedState = JSON.parse(state);
        // Process updated state if needed (optional)
        publish(PUB_SUB_EVENTS.cartUpdate, { source: 'cart-items', cartData: parsedState });
      })
      .catch(() => {
        console.error('Error updating line item properties.');
      })
     .finally(() => {
       if (productId) {
       const associatedProductElement = document.querySelector(`[data-product-id="${productId}"]`);
...
class CartItemGiftUpdate extends HTMLElement {
  ...
  handleUpdateButtonClick(event) {
    ...
    const properties = {
      ...
      gift_from: toName,
      gift_to: fromName,
      gift_massage: giftMessage
    };

    const cartItems = this.closest('cart-items') || this.closest('cart-drawer-items');
    if (cartItems) {
      cartItems.updateLineItemProperties(line, properties);
    }
  }
